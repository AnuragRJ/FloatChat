...
INDEX_URL = f"{BASE_URL}/argo_bio-profile_index.txt.gz"
DAC_URL   = f"{BASE_URL}/dac"

OUT_DIR = Path(r"D:\SIH\Current project Main Restart\chatgpt sih data\data_argo_final\data_netcdf\data_bgc_5yr")
...
def download_index() -> Path:
    local_path = OUT_DIR / "argo_bio-profile_index.txt.gz"
    if local_path.exists() and local_path.stat().st_size > 0:
        print(f"✅ BGC index already present: {local_path}")
        return local_path

    print(f"⬇️ Downloading BGC index from {INDEX_URL}...")
    with session.get(INDEX_URL, stream=True, timeout=60) as r:
        r.raise_for_status()
        with open(local_path, "wb") as f:
            for chunk in r.iter_content(chunk_size=16384):
                f.write(chunk)
    print("✅ BGC index downloaded.")
    return local_path


def get_bgc_file_list(idx_path: Path):
    print("🔍 Parsing BGC index & filtering Indian Ocean (last 5 years)...")
    ...
    print(f"📊 BGC profiles selected: {len(df)}")
...
def main():
    ...
    files = get_bgc_file_list(idx_path)

    if not files:
        print("⚠️ No BGC files matched filters.")
        return

    print(f"🚀 Downloading {len(files)} BGC files with {MAX_WORKERS} workers...")
